...
class Agent:
    ...
    def __init__(self,EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE):
        ...
        # how often to update target network (number of episodes)
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if self.num_steps_taken >= self.minibatch_size and not self.finished_training:
            loss = self.dqn.train_q_network(self.buffer, self.minibatch_size, self.discount) # train network 
            
        number_of_episodes = np.floor(self.num_steps_taken/self.episode_length)
        if number_of_episodes% self.target_update == 0:
    ...
    def _continuous_action_to_discrete(self,continuous_action):
        if np.all(continuous_action == np.array([0,0.02],dtype=np.float32)): # Move up
            ...
            discrete_action = 2
        elif np.all(continuous_action == np.array([0.02*np.cos(np.pi/4),0.02*np.cos(np.pi/4)],dtype=np.float32)): # Move up and right
            discrete_action = 3
        elif np.all(continuous_action == np.array([0.02*np.cos(np.pi/4),-0.02*np.cos(np.pi/4)],dtype=np.float32)): # Move down and right
            discrete_action = 4
    ...
    def _discrete_action_to_continuous(self, discrete_action):
        if discrete_action == 0: # Move up
            ...
            continuous_action = np.array([0,-0.02], dtype=np.float32)
        elif discrete_action == 3: # Move up and right
            continuous_action = np.array([0.02*np.cos(np.pi/4),0.02*np.cos(np.pi/4)],dtype=np.float32)
        elif discrete_action == 4: # Move down and right 
            continuous_action = np.array([0.02*np.cos(np.pi/4),-0.02*np.cos(np.pi/4)],dtype=np.float32)
...
class DQN:
    ...
    def train_q_network(self, buffer, minibatch_length, discount):
        ...
        # sample minibatch
        minibatch = buffer.sample_minibatch(buffer.get_length(), minibatch_length)
        # Calculate the loss for this transition.
        loss = self._calculate_minibatch_loss(minibatch, discount)
