...
def main(EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE):
    ...
    print('EPISODE_LENGTH: {}\nMINIBATCH_SIZE: {}\nDISCOUNT: {}\nEPSILON_DECAY: {}\nTARGET_UPDATE: {}'.format(EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE))
    ...
    plt.yscale('log')
    plt.title('average loss vs episode number (log scale)')   
    plt.savefig('hyperparameter_tests/loss_eplen{}_mbsize{}_disc{}_epdec{}_targup{}.png'.format(EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE),dpi=1000)
    plt.close()
    ...
    environment.draw_policy(policy,EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE)
