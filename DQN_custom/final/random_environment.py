...
class Environment:
    ...
    def draw_policy(self, policy, EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE):
        self.show(self.init_state) # draw environment with agent beginning in initial state
        for i in range(len(policy)-1):
            current_state = policy[i] # get coordinates for start of state-state line
            next_state = policy[i+1] # get coordinates for end of state-state line
        ...
        # show image
        cv2.imwrite('hyperparameter_tests/policy_eplen{}_mbsize{}_disc{}_epdec{}_targup{}.png'.format(EPISODE_LENGTH, MINIBATCH_SIZE, DISCOUNT, EPSILON_DECAY, TARGET_UPDATE), self.image)
        cv2.imshow("Environment", self.image)
